...
resource. A downstream phase reads the protected upstream Terraform state,
projects the Orchestrator's documented dependency JSON shape
(`compartments_output.json`, `network_output.json`, and similar), and passes
the resulting file paths through a temporary `.tfvars.json` file. The
temporary file only binds official Orchestrator dependency variables to JSON
files; it does not define resources or replace the Orchestrator.
